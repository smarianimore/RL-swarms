...
import numpy as np
import random
from environments.slime_dph_2.slime_dynamic_dph import Slime
...
def main(args):
    ...
    if args.render:
        from environments.slime_dph_2.slime_dynamic_dph import SlimeVisualizer
        env_vis = SlimeVisualizer(env.W_pixels, env.H_pixels, **v_params)
    else:
        env_vis = None

    states = params['sniff_patches'] * 2 + 1 if params['obs_type'] == "paper" else (params['sniff_patches'] + 1) * 2 + 1
    ...
    if args.train:
        (
            ...
            scatter_action_dict,
            scatter_reward_dict,
        ) = iql.create_agent(params, l_params, states, n_actions, args.train)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--params_path",
        type=str,
        default="environments/slime_dph_2/config/env-params.json",
        required=False
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="environments/slime_dph_2/config/env_visualizer-params.json",
        required=False
    )
