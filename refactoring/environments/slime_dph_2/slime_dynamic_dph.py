...
class Slime(AECEnv):
    ...
    def __init__(self, seed, render_mode: Optional[str] = None, **kwargs):
        ...
        if self.obs_type == "paper":
            ...
            self._observation_spaces = {
                a: Box(low=0.0, high=np.inf, shape=(self.sniff_patches + 1 + 2,), dtype=np.float32)
                for a in self.possible_agents
            }
        ...
        # Static
        self.ph_positions = [(1/4, 1/4), (3/4, 3/4), (1/4, 3/4), (3/4, 1/4)]
        self.reward_patches = self._find_neighbours(max(self.lay_area - 1, 0))
    ...
    def distance_to_goal(self, agent):
        ...
        if agent['mode'] == 'c': # RED
            ...
            x = min(x1**2, x2**2)
            y = min(y1**2, y2**2)
            dist = np.sqrt(x + y) / self.patch_size # Scalo per la grandezza della patch
        elif agent['mode'] == 's': # BLUE
    ...
    def get_reward(self):
        ...
        if self.learners[self.agent]['mode'] == 'c':
            if self.learners[self.agent]['pos'] in self.reward_patches[self.ph_pos1]:
                if self.learners[self.agent]['pos'] == self.ph_pos1:
                    reward += 100.0
                else:
                    reward += 1.0
            if self.learners[self.agent]['pos'] in self.reward_patches[self.ph_pos2]:
                if self.learners[self.agent]['pos'] == self.ph_pos2:
                    reward -= 100.0
                else:
                    reward -= 1.0
    ...
    def get_reward2(self):
        ...
        if self.learners[self.agent]['mode'] == 'c':
            if self.learners[self.agent]['pos'] == self.ph_pos1:
                reward += 100.0
            elif self.learners[self.agent]['pos'] == self.ph_pos2:
                reward -= 100.0
        elif self.learners[self.agent]['mode'] == 's': 
            if self.learners[self.agent]['pos'] == self.ph_pos2:
                reward += 100.0
    ...
    def step(self, action: int):
        ...
        #    self.do_action1()
        elif action == 1:   # Follow pheromone
            self.do_action2()
    ...
    def process_agent(self, cluster_ticks, rewards_cust):
        ...
        """
        #dist = self.distance_to_goal(self.learners[self.agent])
        #rewards_cust[self.agent].append(-dist)
    ...
    def _get_obs2(self, agent):
        f, _ = self._get_new_positions(self.ph_fov, agent)
        obs = [self.patches[tuple(i)]["chemical"] for i in f]
        tmp1 = any([True if tuple(i) in self.lay_patches[self.ph_pos1] else False for i in f])
        tmp2 = any([True if tuple(i) in self.lay_patches[self.ph_pos2] else False for i in f])
        obs.extend([float(tmp1), float(tmp2)]) 
        #breakpoint()
        return np.array(obs)
    ...
    def _get_obs3(self, agent):
        ...
        f, _ = self._get_new_positions(self.ph_fov, agent)
        f_l = f.tolist()
        f_l.append(list(agent['pos']))
        obs = [self.patches[tuple(i)]["chemical"] for i in f_l]
        tmp1 = any([True if tuple(i) in self.lay_patches[self.ph_pos1] else False for i in f_l])
        tmp2 = any([True if tuple(i) in self.lay_patches[self.ph_pos2] else False for i in f_l])
        obs.extend([float(tmp1), float(tmp2)]) 
        return np.array(obs)
            ...
            if self.obs_type == "paper":
                if pos[0] == 1 and pos[1] == 1: 
                    ...
                    if np.unique(obs).shape[0] == 1:
                        obs_id += np.random.randint(self.sniff_patches) + (idx * self.sniff_patches)
                    else:
                        obs_id += obs.argmax().item() + (idx * self.sniff_patches)
                else:
                    if np.unique(obs).shape[0] == 1:
                        obs_id += np.random.randint(self.sniff_patches) + int(pos[1] * self.sniff_patches)
                    else:
                        obs_id += obs.argmax().item() + int(pos[1] * self.sniff_patches)
            elif self.obs_type == "variation1":
                if pos[0] == 1 and pos[1] == 1: 
                    idx = np.random.randint(len(pos)) 
                    if np.unique(obs).shape[0] == 1:
                        obs_id += np.random.randint(self.sniff_patches + 1) + (idx * (self.sniff_patches + 1))
                    else:
                        obs_id += obs.argmax().item() + (idx * (self.sniff_patches + 1))
                else:
                    if np.unique(obs).shape[0] == 1:
                        obs_id += np.random.randint(self.sniff_patches + 1) + int(pos[1] * (self.sniff_patches + 1))
                    else:
                        obs_id += obs.argmax().item() + int(pos[1] * (self.sniff_patches + 1))
        return obs_id
    
    def lay_double_pheromone_gaussian(self, patches, pos1, pos2):
        """
        ...
        if self.diffuse_radius == 0:
            ...
            grid = gaussian_filter(grid.reshape((self.W, self.H)), sigma=self.diffuse_area, radius=self.diffuse_radius, mode='wrap').flatten()

        for p1, p2 in zip(self.lay_patches[pos1], self.lay_patches[pos2]):
            idx1 = patches[p1]['id']
            idx2 = patches[p2]['id']
            patches[p1]['chemical'] = grid[idx1]
            patches[p2]['chemical'] = grid[idx2]
    ...
    def _find_max_pheromone3(self, agent, observations):
        """
        Following pheromone modeis controlled by param self.follow_mode:
            'det' = follow greatest pheromone
            'prob' = follow greatest pheromone probabilistically (pheromone strength as weight)
        ...
        if idx == (obs.shape[0] - 1):
            ...
            f, direction = self._get_new_positions(self.ph_fov, agent)
            ph_pos = tuple(f[idx])
            if self.sniff_patches + 1 < self.N_DIRS + 1:
                ph_dir = self._get_new_direction(self.sniff_patches, direction, idx)
            else:
                ph_dir = idx
        return ph_val, ph_pos, ph_dir
    ...
    def _find_non_max_pheromone2(self, agent, observations):
        # ATTENZIONE: per alcune configurazioni di sniff-patches
        # questo metodo potrebbe cadere in soluzioni sub-ottimali.
        # Per esempio con sniff-patches=3 e la patch dell'agente, se
        # troviamo feromone nelle 3 patches e niente in quella dell'agente,
        # lui rimmarà li fermo a vita!
        if self.sniff_patches < 5:
            warnings.warn(
                "Warnings: sniff-patches less than 5 could lead to the freezing of the agent's position!"
            )
    ...
    def reset(self, seed=None, return_info=True, options=None):
        ...
        ph_pos = self.ph_positions.copy()
        choice = random.choice(ph_pos)
        self.ph_pos1 = (self.coords[0][0] + self.patch_size * int(self.W * choice[0]), self.coords[0][1] + self.patch_size * int(self.H * choice[1])) 
        ph_pos.remove(choice)
        choice = random.choice(ph_pos)
        self.ph_pos2 = (self.coords[0][0] + self.patch_size * int(self.W * choice[0]), self.coords[0][1] + self.patch_size * int(self.H * choice[1])) 
        ...
        if self.obs_type == "paper":
            ...
            self.observations = {
                a: np.zeros(self.sniff_patches + 1 + 2, dtype=np.float32)
                for a in self.agents
            }
...
class SlimeVisualizer:
    ...
    def render(
        ...
        '''
        chem_ph1 = round(patches[ph_pos1]['chemical']) * self.shade_strength
        pygame.draw.rect(
            self.screen,
            (0, chem_ph1 if chem_ph1 <= 255 else 255, 0),
            pygame.Rect(
                ph_pos1[0] - self.offset,
                ph_pos1[1] - self.offset,
                self.patch_size,
                self.patch_size
            )
        )
        ...
        self.screen.blit(text, text.get_rect(center=ph_pos1))
        if self.show_chem_text and (not sys.gettrace() is None or
                                    patches[ph_pos1]['chemical'] >= self.sniff_threshold):  # if debugging show text everywhere, even 0
            text = self.chemical_font.render(str(round(patches[ph_pos1]['chemical'], 1)), True, GREEN)
            self.screen.blit(text, text.get_rect(center=ph_pos1))
        chem_ph2 = round(patches[ph_pos2]['chemical']) * self.shade_strength
        pygame.draw.rect(
            self.screen,
            (0, chem_ph2 if chem_ph2 <= 255 else 255, 0),
            pygame.Rect(
                ph_pos2[0] - self.offset,
                ph_pos2[1] - self.offset,
                self.patch_size,
                self.patch_size
            )
        )
        ...
        self.screen.blit(text, text.get_rect(center=ph_pos2))
        if self.show_chem_text and (not sys.gettrace() is None or
                                    patches[ph_pos2]['chemical'] >= self.sniff_threshold):  # if debugging show text everywhere, even 0
            text = self.chemical_font.render(str(round(patches[ph_pos2]['chemical'], 1)), True, YELLOW)
            self.screen.blit(text, text.get_rect(center=ph_pos2))
        '''
